...
def gen_code(String body) {
    """
        new Code() {
            @Override
            void execute(UM um) { $body }
        }"""
}

def one_param_opcode(Closure gen_statement) {
    (0..7).collect { c -> gen_code(gen_statement(c)) }.join(",\n")
}

def two_param_opcode(Closure gen_statement) {
    (0..63).collect { bc -> gen_code(gen_statement(bc >>> 3, bc & 7)) } .join(",\n")
}

def three_param_opcode(Closure gen_statement) {
    (0..511).collect { abc -> gen_code(gen_statement(abc >>> 6, (abc >>> 3) & 7, abc & 7)) }.join(",\n")
}
...
def gen_orthography(a) {
    (0..<ORTHOGRAPHY_VALUE_LIMIT).collect { c -> gen_code("um.r$a = $c;") }.join(",\n")
}
    ...
    public static Code forOp(int op) {
        switch (op >>> 28) {
            ...
            case 13 : // Orthography
                ...
                switch (a) {
                ${(0..7).collect { r -> """
                    case $r : return ${ gen_code("um.r$r = v;") };""" }.join() }
                }
            default:
                return illegal;
        }
